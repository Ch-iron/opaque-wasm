...
import initOpaqueWasm, { Login, Registration } from 'opaque-client';
...
export async function signup({ username, password }: AuthFormValues) {
  ...
  const serverSPK = hex.encode(registration.getServerSPK());
  console.dir({
    exportKey,
    serverSPK,
  });
...
export async function login({ username, password }: AuthFormValues) {
  ...
  const serverSPK = hex.encode(login.getServerSPK());
  console.dir({
    exportKey,
    sessionKey,
    serverSPK,
